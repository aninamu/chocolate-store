...
|---------|------|---------|
| MongoDB 7 | 57017 | Primary data store (user-level `mongod`, data in `.data/mongo/`, single-node replica set) |
...
`make setup` and `make dev` expect **MongoDB 7**, **Redis 7**, and **`python3.12-venv`** on the VM (see README install notes). Bootstrap also runs `pip install` and `npm install`, which need working egress to PyPI and the npm registry.
...
- MongoDB runs as a **single-node replica set** (`--replSet rs0`) so checkout can use multi-document transactions.
- The database is **dropped and reseeded** on every `make services-up` / `make dev` — collections come from Beanie models, seed data from `backend/app/seed.py`.
...
- The `.env` file is auto-created from `.env.example` on first bootstrap. Ports are non-standard (57017, 63790) to avoid collisions with system services.

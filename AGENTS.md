...
| Axum backend | 8000 | REST API (`cargo run --bin chocolate-store-api`) |
...
`make setup` and `make dev` expect **Postgres 16**, **Redis 7**, and a **Rust toolchain** (`cargo`) on the VM (see README apt packages). Bootstrap runs `cargo build` and `npm install`, which need working egress to crates.io and the npm registry.
...
- `crates.io`, `static.crates.io` (Rust dependencies)
...
If you just approved outbound domains and installs still fail with TLS errors (cargo/npm) or apt `404`/`Release file` errors, **start a new Cloud Agent session** so the VM picks up the updated network policy, then run `make setup` again.

...
function TagMultiselectDropdown({
  ...
  }, [open, closeWithFocusReturn]);

  useEffect(() => {
    if (!open || isPending || tagOptions.length === 0) return;
  ...
  }, [open, isPending, tagOptions.length]);
  ...
  return (
    <div ref={rootRef} className="relative min-w-0">
      <Button
        ...
        aria-labelledby={`${tagsHeadingId} ${triggerId}-summary`}
        onClick={() => {
          if (!isPending && !empty) setOpen((v) => !v);
        }}
      >
        <span
          id={`${triggerId}-summary`}

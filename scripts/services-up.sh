...
# schema.sql and load seed data. No migrations — the DB is disposable.
set -euo pipefail
...
# ---- Fresh app DB every startup (no migrations; schema comes from backend/schema.sql)
...
( cd "$ROOT/backend" && cargo run --quiet --bin init-db )

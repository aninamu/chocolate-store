#!/usr/bin/env bash
# Run backend (cargo run) and frontend (next dev) in parallel.
...
( cd "$ROOT/backend" && cargo run --quiet --bin chocolate-store-api 2>&1 | while IFS= read -r line; do echo "[backend]  $line"; done ) &

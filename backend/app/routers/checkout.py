...
async def checkout(
    ...
    async with session.begin():
        ...
        line_ids = [line.chocolate_id for line in body.items]
        res = await session.execute(
            select(Chocolate).where(Chocolate.id.in_(line_ids))
        )
        chocolates_by_id = {ch.id: ch for ch in res.scalars().all()}

        for line in body.items:
            ch = chocolates_by_id.get(line.chocolate_id)
            if ch is None:
                raise HTTPException(

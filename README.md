...
A field-engineer friendly demo: browse a chocolate marketplace, save items, use a local cart, and run a **mock** checkout. No user accounts. **Next.js 15 (App Router + Tailwind + shadcn/ui)**, **Rust (Axum)**, **Postgres** (user-level `pg_ctl`, project-local data dir), **Redis** (user-level `redis-server`, local port), all in **dev mode with hot reload**.
...
Optional: `cargo install cargo-watch` for backend hot reload during `make dev`.
...
1. **Bootstrap** — run `cargo fetch` for the API, `npm install` the frontend, copy `.env.example` → `.env` if needed, and `initdb` a Postgres cluster in `./.data/postgres` the first time.
...
3. **Run both apps** — Axum API on **8000** (`cargo watch` / `cargo run`), Next on **3000** with `next dev`.
...
| Target | What it does |
|--------|----------------|
| `make setup` | Bootstrap only (cargo fetch, npm, .env, `initdb` if new). |
...
| `make test-backend-coverage` | Backend tests with coverage → `backend/coverage/` (requires `cargo-llvm-cov`). |
| `make test-frontend` | Frontend tests only. |
| `make test-frontend-coverage` | Frontend tests with coverage → `frontend/coverage/`. |

### Tests and coverage

- **Backend:** from the repo root, `make setup` then `make services-up` so Postgres (**55432**) and Redis (**63790**) are listening. API integration tests skip automatically if those ports are closed. Coverage HTML: [`backend/coverage/index.html`](./backend/coverage/index.html) after `make test-backend-coverage`.
...
Copy and edit [`.env.example`](./.env.example) to `.env` (done automatically on first `make dev`). The API reads `DATABASE_URL` and `REDIS_URL` (SQLAlchemy-style `postgresql+asyncpg://` URLs are normalized automatically); the browser uses `NEXT_PUBLIC_API_URL` (also exported in `scripts/dev.sh` for `next dev`).
...
3. Tweak a response in `backend/src/routes/chocolates.rs` (e.g. an extra field on `ChocolateOut`) — `cargo watch` should pick it up; refresh the list.
...
- [`backend/`](./backend) — Axum + sqlx. Schema in [`src/schema.sql`](./backend/src/schema.sql); catalog data in [`src/seed.rs`](./backend/src/seed.rs); loaded by the `init-db` binary.

...
- **macOS (Homebrew):** `brew install mongodb-community redis`
- **Ubuntu/Debian:** `sudo apt-get install mongodb redis-server python3.12-venv`
...
1. **Bootstrap** — create `backend/.venv`, `pip install` the API, `npm install` the frontend, copy `.env.example` → `.env` if needed.
2. **Bring data stores up** — start MongoDB on **57017** and Redis on **63790** (isolated from any system services), drop and reseed the app database, then load [`backend/app/seed.py`](./backend/app/seed.py). The database is wiped on every start/stop, so editing `seed.py` is all you need to change the catalog.
...
| Target | What it does |
|--------|----------------|
| `make setup` | Bootstrap only (venv, npm, .env). |
...
- [`backend/`](./backend) — FastAPI, Motor (async MongoDB). Collections are created and seeded each run by [`app/init_db.py`](./backend/app/init_db.py); catalog data lives in [`app/seed.py`](./backend/app/seed.py).
